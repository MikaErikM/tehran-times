...
class StateManager:
    ...
    def save_checkpoint(self, date: datetime, page: int):
...
class ArchiveCrawler:
    ...
    def __init__(self, output_dir: str, debug: bool = False, dry_run: bool = False):
        self.base_url = "https://www.tehrantimes.com/page/archive.xhtml"
        ...
        self.state_manager = StateManager(self.output_dir / 'checkpoints')
    ...
    def _setup_directories(self):
        ...
        self.failed_file = self.output_dir / 'failed' / 'failed_pages.csv'
        self.access_log = self.output_dir / 'logs' / 'access.log'
    ...
    def _setup_logging(self):
        ...
        log_file = self.output_dir / 'logs' / f'crawler_{datetime.now():%Y%m%d_%H%M%S}.log'
    ...
    def log_access(self, date: datetime, page: int, status: int, response_time: float):
    ...
    def log_failed_page(self, date: datetime, page: int, error: str):
    ...
    def save_articles(self, date: datetime, articles: List[Dict]):
        """Save articles with validation."""
        if not articles:
            return
            
        filename = self.output_dir / 'data' / f'articles_{date:%Y%m%d}.json'
    ...
    def scrape_page(self, date: datetime, page: int) -> List[Dict]:
        ...
        url = f"{self.base_url}?mn={date.month}&dy={date.day}&yr={date.year}&pi={page}"
        ...
        with tqdm(dates, desc="Crawling dates") as pbar:
            for date in pbar:
                ...
                for page in range(1, 6):  # Assuming max 5 pages per date
                    ...
                    page_articles = self.scrape_page(date, page)
...
def main():
    ...
    crawler = ArchiveCrawler(args.output, args.debug, args.dry_run)
